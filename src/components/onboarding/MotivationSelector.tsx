...
export default function MotivationSelector({ selectedMotivations, onUpdate }: MotivationSelectorProps) {
    ...
    <div className="space-y-6">
      <div>
        <p className="text-gray-600 mb-4">
          Select what motivates your GAA travel - then drag to rank by importance!
        </p>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {unselectedMotivations.map((motivation, index) => (
            <motion.button
              ...
              className="relative p-3 rounded-lg border-2 transition-all text-center border-gray-200 hover:border-gray-300 bg-white hover:bg-gray-50"
            >
              ...
              {(provided, snapshot) => (
                ...
                >
                  {selectedMotivations.map((motivationId, index) => {
                    ...
                    return (
                      <Draggable key={motivation.id} draggableId={motivation.id} index={index}>
                        {(provided, snapshot) => (
                          <div
                            ref={provided.innerRef}
      ...
      <p className="text-sm text-gray-500 text-center">
        Selected: {selectedMotivations.length} {selectedMotivations.length === 1 ? 'motivation' : 'motivations'}
      </p>
    </div>
  );
}