...
export default function TestimonialsDashboardWidget({ clubId }: TestimonialsDashboardWidgetProps) {
  const [summary, setSummary] = useState<TestimonialSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [processingId, setProcessingId] = useState<string | null>(null);

  const buildSummaryFromTestimonials = (testimonials: ApiTestimonial[]): TestimonialSummary => {
    return {
      pending: testimonials.filter((t) => t.status === 'PENDING').length,
      superAdminApproved: testimonials.filter((t) => t.status === 'SUPER_ADMIN_APPROVED').length,
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-yellow-50 rounded-lg p-3">
            <div className="text-2xl font-bold text-yellow-600">{summary.superAdminApproved}</div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-yellow-700 mt-1">
                  You have {summary.superAdminApproved} testimonial{summary.superAdminApproved !== 1 ? 's' : ''} awaiting your approval.
                </p>
                <Link
                  href={`/club-admin/${clubId}/testimonials`}
        ...
        {/* Recent Testimonials Awaiting Approval */}
        {summary.recentTestimonials.length > 0 && (
          <div>
            <h3 className="text-sm font-medium text-gray-900 mb-3">Recent Testimonials Awaiting Your Approval</h3>
        ...
        {summary.total === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">
              Testimonials from your club members will appear here once submitted.
            </p>
          </div>
        )}
      </div>
