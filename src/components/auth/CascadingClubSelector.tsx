...
// Sports options - currently unused but kept for potential future use
...
export default function CascadingClubSelector({
  value,
  onChange,
}: CascadingClubSelectorProps) {
  // Toggle state
  ...
  const loadRegions = async (countryId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/clubs/regions-sql?countryId=${countryId}`
      );
      if (response.ok) {
        const data = await response.json();
        setRegions(data);
  ...
  const loadBritainRegions = async () => {
    ...
    try {
      ...
      const britainUnit = units.find(
        (u: InternationalUnit) => u.code === "BRITAIN"
      );
      if (!britainUnit) throw new Error("Britain unit not found");

      const countryResponse = await fetch(
        `/api/clubs/countries-sql?unitId=${britainUnit.id}`
      );
      if (!countryResponse.ok)
        throw new Error("Failed to load Britain country");
      const countries = await countryResponse.json();
      const britainCountry = countries.find(
        (c: Country) => c.code === "BRITAIN"
      );
      if (!britainCountry) throw new Error("Britain country not found");

      // Load Britain regions
      const response = await fetch(
        `/api/clubs/regions-sql?countryId=${britainCountry.id}`
      );
      if (response.ok) {
        const data = await response.json();
        setRegions(data);
  ...
  const loadBritainClubs = async (regionId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/clubs/filtered-sql?${params.toString()}`
      );
      if (response.ok) {
        const data = await response.json();
        setClubs(data);
  ...
  const loadNewYorkClubs = async () => {
    ...
    try {
      ...
      if (!unitResponse.ok) throw new Error("Failed to load units");
      const units = await unitResponse.json();
      const newYorkUnit = units.find(
        (u: InternationalUnit) => u.code === "NEW_YORK"
      );
      if (!newYorkUnit) throw new Error("New York unit not found");

      const countryResponse = await fetch(
        `/api/clubs/countries-sql?unitId=${newYorkUnit.id}`
      );
      if (!countryResponse.ok)
        throw new Error("Failed to load New York countries");
      const countries = await countryResponse.json();
      ...
      const response = await fetch(
        `/api/clubs/filtered-sql?countryId=${usaCountry.id}`
      );
      if (response.ok) {
        const data = await response.json();
        setClubs(data);
  ...
  const loadUSAGAADivisions = async () => {
    ...
    try {
      ...
      if (!unitResponse.ok) throw new Error("Failed to load units");
      const units = await unitResponse.json();
      const usaGaaUnit = units.find(
        (u: InternationalUnit) => u.code === "USA_GAA"
      );
      if (!usaGaaUnit) throw new Error("USA GAA unit not found");

      const countryResponse = await fetch(
        `/api/clubs/countries-sql?unitId=${usaGaaUnit.id}`
      );
      if (!countryResponse.ok)
        throw new Error("Failed to load USA GAA countries");
      const countries = await countryResponse.json();
      ...
      const response = await fetch(
        `/api/clubs/regions-sql?countryId=${usaCountry.id}`
      );
      if (response.ok) {
        const data = await response.json();
        setRegions(data);
  ...
  const loadCanadaDivisions = async () => {
    ...
    try {
      ...
      if (!unitResponse.ok) throw new Error("Failed to load units");
      const units = await unitResponse.json();
      const canadaUnit = units.find(
        (u: InternationalUnit) => u.code === "CANADA"
      );
      if (!canadaUnit) throw new Error("Canada unit not found");

      const countryResponse = await fetch(
        `/api/clubs/countries-sql?unitId=${canadaUnit.id}`
      );
      if (!countryResponse.ok)
        throw new Error("Failed to load Canada countries");
      const countries = await countryResponse.json();
      const canadaCountry = countries.find((c: Country) => c.code === "CAN");
      if (!canadaCountry)
        throw new Error("Canada country not found under Canada unit");

      // Load divisions (regions)
      const response = await fetch(
        `/api/clubs/regions-sql?countryId=${canadaCountry.id}`
      );
      if (response.ok) {
        const data = await response.json();
        setRegions(data);
  ...
  const handleManualSubmit = async () => {
    if (
      !manualSubmission.name ||
      !manualSubmission.country ||
      !manualSubmission.primaryContactName ||
      !manualSubmission.primaryContactEmail
    ) {
      toast.error(
    ...
    <div className="space-y-6">
      ...
      {isClubMember === true && (
        <div className="space-y-4">
          ...
          {((selectedCountry &&
            !isIreland() &&
            countries.find((c) => c.id === selectedCountry)?.hasRegions) ||
            (isBritain() && regions.length > 0) ||
            (isUSAGAA() && regions.length > 0) ||
            (isCanada() && regions.length > 0)) &&
            !isNewYork() && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Step {isBritain() || isUSAGAA() || isCanada() ? "2" : "3"}:
                  Select {isUSAGAA() || isCanada() ? "Division" : "Region"}
                </label>
                <select
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <button
                    ...
                    className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-medium rounded-lg transition-colors cursor-pointer"
            ...
            <div className="mt-6 p-6 bg-white border-2 border-gray-200 rounded-xl">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={manualSubmission.country}
                    onChange={(e) =>
                      setManualSubmission({
                        ...manualSubmission,
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={manualSubmission.cityOrRegion}
                    onChange={(e) =>
                      setManualSubmission({
                        ...manualSubmission,
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={manualSubmission.primaryContactName}
                    onChange={(e) =>
                      setManualSubmission({
                        ...manualSubmission,
                ...
                <div>
                  ...
                  <input
                    type="email"
                    value={manualSubmission.primaryContactEmail}
                    onChange={(e) =>
                      setManualSubmission({
                        ...manualSubmission,
                ...
                <div>
                  ...
                  <textarea
                    rows={3}
                    value={manualSubmission.notesForAdmin}
                    onChange={(e) =>
                      setManualSubmission({
                        ...manualSubmission,
                        notesForAdmin: e.target.value,
                      })
                    }
                    className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:border-green-600 focus:ring-2 focus:ring-green-100"
                    placeholder="Any additional information for the admin team (optional)"
                  />
                </div>
