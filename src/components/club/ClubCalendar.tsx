...
export default function ClubCalendar({ clubId, clubName }: ClubCalendarProps) {
  ...
  const getDateContent = (date: Date) => {
    ...
    const availability = availabilitySlots.find(slot => 
  ...
  const monthInterests = interests.filter(interest => 
    interest.interestType === 'ENTIRE_MONTH' || interest.interestType === 'MULTIPLE_MONTHS'
  );
        ...
        {monthInterests.length > 0 && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-sm text-yellow-800">
              <strong>{monthInterests.length} team{monthInterests.length > 1 ? 's' : ''}</strong> interested in visiting during {format(currentDate, 'MMMM yyyy')}
            </p>
          </div>
        )}
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Team size: {interest.teamSize} | Flexibility: {interest.flexibility}
                  </p>
                  {interest.interestType === 'SPECIFIC_DATE' && interest.specificDate && (
                    <p className="text-sm text-gray-600">
