...
describe("useAnalytics", () => {
  ...
  describe("trackTournamentInterest", () => {
    it("should track tournament interest with dates", () => {
      ...
      result.current.trackTournamentInterest("tour123", "Test Tournament", [
        "2024-06-01",
        "2024-06-02",
      ]);
  ...
  describe("trackOnboardingComplete", () => {
    it("should track onboarding completion with preferences", () => {
      const { result } = renderHook(() => useAnalytics());

      result.current.trackOnboardingComplete({
        motivations: ["sun_beach", "city_break"],
        competitiveLevels: ["friendly_tournament", "social_gaa"],
        preferredCities: ["Barcelona", "Lisbon"],
        preferredCountries: ["Spain", "Portugal"],
      });

      expect(mockTrackEvent).toHaveBeenCalledWith("onboarding_complete", {
        motivations_count: 2,
        competitive_levels_count: 2,
        cities_count: 2,
        countries_count: 2,
      });
    });
