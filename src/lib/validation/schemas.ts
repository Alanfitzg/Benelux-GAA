...
export const usernameSchema = z
  ...
  .refine(
    (val) => !val.includes("_-") && !val.includes("-_"),
    "Username cannot contain underscore-hyphen combinations"
  )
...
const EventBaseSchema = z.object({
  ...
  location: z
    ...
    .max(500, "Location must be no more than 500 characters"),
...
export const ClubCreateSchema = z.object({
  name: z
    ...
    .max(200, "Club name must be no more than 200 characters"),
...
export const ClubUpdateSchema = ClubCreateSchema.partial().merge(
  z.object({
...
export const ContactFormSchema = z.object({
  ...
  subject: z
    ...
    .max(200, "Subject must be no more than 200 characters"),
  message: z
    .string()
...
export function validateTeamTypes(teamTypes: string[]) {
  ...
  const invalidTypes = teamTypes.filter(
