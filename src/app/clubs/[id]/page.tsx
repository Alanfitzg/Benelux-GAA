...
import ClubTravelStats from "@/components/club/ClubTravelStats";
...
export default async function ClubDetailsPage({
  ...
  // Query travel stats for non-European clubs (Irish/British clubs that travel)
  const travelStatsData = await prisma.tournamentTeam.findMany({
    where: {
      clubId: id,
      event: {
        approvalStatus: "APPROVED",
        startDate: {
          lt: new Date(), // Only past events count as "completed"
        },
      },
    },
    select: {
  ...
  const uniqueEventIds = new Set(travelStatsData.map((t) => t.event.id));

  // Extract cities (first part of location before comma)
  const uniqueCities = new Set(
    travelStatsData
      .map((t) => {
        const location = t.event.location;
        if (!location) return null;
        // City is typically the first part before comma
        return location.split(",")[0].trim();
      })
      .filter(Boolean) as string[]
  );

  // Extract countries (last part of location after last comma)
  ...
  const travelStats = {
    ...
    tripsCompleted: uniqueEventIds.size,
    tournamentsAttended: uniqueEventIds.size,
  };
  ...
  return (
    <>
      ...
      {/* Cover Photo Banner - Only for European clubs */}
      {club.isMainlandEurope && <ClubCoverPhotoBanner clubId={club.id} />}

      {/* Irish/British clubs: Full navy background with white container for entire profile */}
      {!club.isMainlandEurope ? (
        <div className="bg-gradient-to-br from-[#264673] to-[#1a3352] py-8 min-h-[80vh]">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                  ...
                  <div className="flex flex-row items-start gap-4 sm:gap-6">
                    ...
                    <div className="flex-1 min-w-0 pr-2">
                      ...
                      <div className="flex items-start gap-2 sm:gap-3 mb-1 sm:mb-2">
                        <h1 className="text-xl sm:text-3xl md:text-4xl font-bold text-gray-900 line-clamp-2 sm:line-clamp-1">
                          {club.name}
                        </h1>
                        {club.verificationStatus === "VERIFIED" && (
                          <VerifiedTooltip>
                      ...
                      <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-gray-600 text-sm sm:text-base">
                        {club.clubType && club.clubType !== "CLUB" && (
                          <span className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-0.5 sm:py-1 bg-primary/10 text-primary text-xs sm:text-sm font-medium rounded-full">
                            <svg
                      ...
                      {/* Sports Badges (hidden on mobile) */}
                      {club.teamTypes && club.teamTypes.length > 0 && (
                        <div className="mt-3 hidden sm:block">
                          <SportsBadges teamTypes={club.teamTypes} />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex-shrink-0 flex flex-col items-center">
                      <Image
                        ...
                        width={140}
                        height={140}
                        className="w-20 h-20 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-xl object-contain bg-gray-50 p-2 sm:p-3 shadow-sm border border-gray-100"
                        unoptimized
                      />
                ...
                {/* Content sections inside white container */}
        ...
        /* European clubs: Original layout with blue header banner */
        <>
          ...
          <div className="bg-gray-200 py-6">
            <div className="container mx-auto px-4">
              <div className="max-w-6xl mx-auto space-y-8">
                {/* Hero Section: Tournaments + Visit Card sidebar */}
                <section id="events" className="scroll-mt-24">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                    {/* Visit Club Card - Mobile Only */}
