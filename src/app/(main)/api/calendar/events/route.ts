...
export async function GET(request: NextRequest) {
  try {
    ...
    const where: { clubId: string; startDate?: { gte: Date; lte: Date } } = {
      clubId,
    };
    ...
    const eventsWithConflicts = await Promise.all(
      events.map(async (event) => {
        if (
          event.eventSource === EventSource.FIXTURE &&
          event.fixtureType === FixtureType.COMPETITIVE
        ) {
          return event;
        }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      validatedData.eventSource === EventSource.FIXTURE &&
      validatedData.fixtureType === FixtureType.COMPETITIVE
    ) {
...
async function createConflictNotification(clubId: string, date: string) {
  ...
  console.log(
    `Conflict notification: Club ${clubId} created event on ${date} with existing fixture`
  );
}
