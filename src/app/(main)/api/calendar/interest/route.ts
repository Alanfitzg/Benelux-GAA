...
export async function GET(request: NextRequest) {
  try {
    ...
    const where: { clubId: string; date?: { gte: Date; lte: Date } } = {
      clubId,
    };
    ...
    const interestsByDate = interests.reduce(
      (acc, interest) => {
        const dateKey = interest.date.toISOString().split("T")[0];

        // Check if this date is blocked
        const isBlocked = blockedWeekends.some(
          (blocked) =>
            interest.date >= blocked.startDate &&
            interest.date <= blocked.endDate
        );
        ...
        if (!acc[dateKey]) {
          acc[dateKey] = {
            ...
            uniqueUsers: new Set(),
            locations: [],
            userIds: permissions.canViewInterestIdentities
              ? new Set()
              : undefined,
          };
        }
    ...
    const formattedInterests = await Promise.all(
      Object.values(interestsByDate).map(
        async (item: {
          date: Date;
          totalSubmissions: number;
          uniqueUsers: Set<string>;
    ...
    if (existingInterest) {
      ...
      interest = await prisma.calendarInterest.update({
        ...
        data: {
          ...
          preferredLocation:
            validatedData.preferredLocation ||
            existingInterest.preferredLocation,
        },
      });
    } else {
      // Create new interest
