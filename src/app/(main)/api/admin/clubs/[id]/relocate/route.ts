...
export async function PATCH(
  ...
  try {
    ...
    if (internationalUnitId) {
      ...
      if (!internationalUnit) {
        return NextResponse.json(
          { error: "Invalid international unit" },
          { status: 400 }
        );
      }
    }
    ...
    if (countryId) {
      ...
      if (
        internationalUnitId &&
        country.internationalUnitId !== internationalUnitId
      ) {
        return NextResponse.json(
          {
            error:
              "Country does not belong to the specified international unit",
          },
          { status: 400 }
        );
      }
    ...
    console.log(
      `Club "${existingClub.name}" relocated by ${session.user.email} from:`,
      {
