  ...
  try {
    ...
    if (!event) {
      return NextResponse.json(
        { error: MESSAGES.ERROR.EVENT_NOT_FOUND },
        { status: 404 }
      );
    }
    ...
    try {
      ...
      console.warn(
        "PitchLocations table not found, returning event without pitch data"
      );
    }
...
export async function PUT(
  ...
  try {
        ...
        try {
          ...
          if (body.pitchLocationIds.length > 0) {
            await tx.eventPitchLocation.createMany({
              data: body.pitchLocationIds.map(
                (pitchId: string, index: number) => ({
                  eventId: id,
    ...
    return NextResponse.json(
      { error: MESSAGES.ERROR.GENERIC, details: errorMessage },
      { status: 400 }
    );
  }
}
