...
interface Match {
  stage: "Cup Final" | "3rd/4th Playoff" | "4th/5th Playoff";
...
const matches: Match[] = [
  {
    stage: "Cup Final",
    teamA: { name: "Brussels", score: "2-12", points: 18 },
    teamB: { name: "Amsterdam", score: "0-07", points: 7 },
    status: "final",
    winner: "A",
  },
  {
    stage: "3rd/4th Playoff",
    teamA: { name: "Eindhoven", score: "", points: 0 },
    teamB: { name: "Den Haag", score: "", points: 0 },
    status: "pending",
  },
  {
    stage: "4th/5th Playoff",
    teamA: { name: "Den Haag", score: "5-02", points: 17 },
    teamB: { name: "Paris", score: "2-06", points: 12 },
    status: "final",
    winner: "A",
  },
];
...
const finalRanking: FinalRank[] = [
  ...
  },
  {
    ...
    name: "Eindhoven",
    crest: "/club-crests/benelux-eindhoven-shamrocks.png",
    note: "3rd/4th pending",
  },
  {
    rank: 4,
    name: "Den Haag",
    crest: "/club-crests/benelux-den-haag.png",
    note: "4/5 Playoff won",
  },
  {
    rank: 5,
    name: "Paris",
...
function MatchRow({ match }: { match: Match }) {
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3 py-2 sm:py-2.5 border-b border-gray-100 last:border-b-0">
      ...
      <div
        className={`flex-1 flex items-center justify-end gap-2 min-w-0 ${winnerA ? "font-semibold text-gray-900" : pending ? "text-gray-500" : "text-gray-500"}`}
      >
      ...
      <div className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm tabular-nums flex-shrink-0">
        {pending ? (
          ...
          <>
            <span
              className={winnerA ? "font-bold text-gray-900" : "text-gray-500"}
            >
              {match.teamA.points}
            </span>
            <span className="text-gray-300">·</span>
            <span
              className={winnerB ? "font-bold text-gray-900" : "text-gray-500"}
            >
              {match.teamB.points}
            </span>
          </>
      ...
      <div
        className={`flex-1 flex items-center gap-2 min-w-0 ${winnerB ? "font-semibold text-gray-900" : pending ? "text-gray-500" : "text-gray-500"}`}
      >
...
export default function BreaghHurlingChampionship() {
  return (
    <div className="mb-6 sm:mb-8 bg-white rounded-xl sm:rounded-2xl shadow-lg border-2 border-[#2B9EB3]/30 overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <h2 className="text-base sm:text-2xl font-extrabold tracking-tight leading-tight">
              2026 Regional Hurling Championship
            </h2>
            <p className="text-white/60 text-[10px] sm:text-sm mt-0.5 sm:mt-1">
              The Hague · 28 March · 9s format
            </p>
          </div>
          <a
      ...
      <div className="bg-gray-50 px-3 sm:px-5 py-3 text-[11px] sm:text-xs text-gray-500 leading-relaxed border-t border-gray-200">
        ...
        knockout finals. Paris hosted as guests due to limited hurling in
        France.
      </div>
    </div>
  );
