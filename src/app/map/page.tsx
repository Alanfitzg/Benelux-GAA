...
export default function MapPage() {
  ...
  const handleClubClick = (club: Club) => {
    if (club.latitude !== undefined && club.longitude !== undefined) {
      setViewState({ longitude: club.longitude, latitude: club.latitude, zoom: 8 });
              ...
              <ul className="space-y-4">
                {clubs.map((club) => (
                  ...
                  >
                    {/* Avatar/Icon */}
                    <Image
                      src={club.imageUrl || "https://gaelic-trips-bucket.s3.eu-west-1.amazonaws.com/tournament-icon.jpg"}
                      alt={club.name}
                      width={40}
                      height={40}
                      className="w-10 h-10 rounded-full object-contain bg-white border"
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-400">{club.location}</div>
                    </div>
                    {selectedClubId === club.id && (
                      <Link
                        href={`/clubs/${club.id}`}
      ...
      <div className="flex-1 relative">
        ...
        >
          {viewMode === 'event' ? (
            ...
            // Club markers
            clubs.map((club) => {
              if (club.latitude === undefined || club.longitude === undefined) return null;
              return (
                <Marker
                  key={club.id}
          ...
          {viewMode === 'event' ? (
            ...
            clubs.map((club) => {
              ...
              return (
                ...
                >
                  <div className="p-2 min-w-[180px]">
                    <div className="font-bold text-base mb-1">{club.name}</div>
                    <div className="text-xs text-gray-500 mb-1">
                      {club.location}
                    </div>
                    <div className="text-xs text-gray-500 mb-2">
