    ...
    if (regionId) {
      ...
      if (search && search.length >= 2) {
        clubs = await prisma.$queryRaw`
          SELECT id, name, location, "sportsSupported", "verificationStatus", "dataSource", "countryId", "regionId"
          FROM "Club"
...
export const GET = withRateLimit(RATE_LIMITS.PUBLIC_API, getFilteredClubsHandlerSQL);