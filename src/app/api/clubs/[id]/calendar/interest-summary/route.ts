...
export async function GET(
  ...
  try {
    ...
    interests.forEach((interest) => {
      ...
      } else if (interest.interestType === 'ENTIRE_MONTH' && interest.monthYear) {
        monthsAffected.push(format(interest.monthYear, 'yyyy-MM'));
      } else if (interest.interestType === 'MULTIPLE_MONTHS' && interest.monthYear) {
        monthsAffected.push(format(interest.monthYear, 'yyyy-MM'));
      } else if (interest.interestType === 'DATE_RANGE' && interest.dateRangeStart && interest.dateRangeEnd) {
    ...
    const summary = Object.entries(monthlyStats).map(([month, stats]) => ({
      month,
      ...stats,
      averageTeamSize: stats.total > 0 ? Math.round(stats.totalTeamSize / stats.total) : 0,
    }));
