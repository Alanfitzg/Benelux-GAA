...
export default async function AdminClubsPage() {
  ...
  const clubs = await prisma.club.findMany({ orderBy: { name: 'asc' }, select: { id: true, name: true, map: true, imageUrl: true, region: true, subRegion: true, location: true, facebook: true, instagram: true, website: true, codes: true } });
  return (
    <div>
        ...
        {clubs.map((club: { id: string; name: string; region: string | null; subRegion: string | null; map: string | null; facebook: string | null; instagram: string | null; website: string | null; codes: string | null; imageUrl: string | null; location: string | null; }) => (
          <div key={club.id} className="bg-white p-4 rounded shadow">
            ...
            <div className="mt-2 space-x-2">
              <Link href={`/admin/clubs/${club.id}/edit`} className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">Edit</Link>
