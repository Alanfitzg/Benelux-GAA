...
export const useAnalytics = () => {
  ...
  const trackClubView = useCallback(
    (clubId: string, clubName: string, clubLocation?: string) => {
  ...
  const trackClubAdminRequest = useCallback(
  ...
  const trackClubContact = useCallback(
    (
      clubId: string,
      clubName: string,
  ...
  const trackEventView = useCallback(
    (eventId: string, eventName: string, eventType?: string) => {
  ...
  const trackTournamentInterest = useCallback(
    (tournamentId: string, tournamentName: string, selectedDates: string[]) => {
  ...
  const trackOnboardingComplete = useCallback(
    (preferences: {
      ...
      trackEvent(GA_EVENTS.ONBOARDING_COMPLETE, {
        ...
        competitive_levels_count: preferences.competitiveLevels.length,
  ...
  const trackSurveySubmit = useCallback(
    (surveyType: string, responses: Record<string, unknown>) => {
  ...
  const trackSocialShare = useCallback(
    (method: string, contentType: "club" | "event", itemId: string) => {
